...
- Решает проблему TCP-рукопожатия, которое режется с помощью технических средств ограничения траффика
  - Скрипт ограничивает частоту входящих SYN на 1/сек. с одного IP, так как тех. средства ограничивают TCP соединение только если их >1 в секунду.
- iOS отдельно
  - У iOS в отличии от Android и Desktop разные паттерны подключений. В одном лимите они мешают друг другу. Разделение на порты конечно решение, но костыльное. Наш фикс производит разделение этих клиентов по TTL, таки образом с одного порта могут сидеть клиенты любых устройств без лишней мороки
...
- В MSS просто нет необходимости для данного билда, поэтому в скрипте добавлена функция его отключения.
...
Важно также сказать, что все тесты проводились без сторонних надстроек - хапрокси, амнезия,даблхоп и тд., поэтому с какими-либо изменениями может не работать, дайте нам знать по поводу возникших проблем, если есть способ это исправить - сообщите нам, попробуем что-то с этим сделать. Если же вы нашли решение сами - аналогично сообщите нам ;)
